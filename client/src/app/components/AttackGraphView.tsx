...
import { Network, Shield, AlertTriangle, ArrowRight, Play, Info, Server, Cpu, CheckCircle2, Eye, RefreshCw } from "lucide-react";
...
export function AttackGraphView({ graph, onSimulatePath, simulationResult }: AttackGraphViewProps) {
  ...
  const [entryHostId, setEntryHostId] = useState<string>(nodes[0]?.ip || targetIp);
  const [entryVuln, setEntryVuln] = useState<string>("Weak Credentials / Service Exposure");
  const [localSimulation, setLocalSimulation] = useState<any>(null);

  useEffect(() => {
    if (nodes.length > 0) {
      setSelectedNode(nodes[0]);
      setEntryHostId(nodes[0].ip);
    }
  }, [useDemoData, graph]);
    ...
    <div className="mx-5 mb-5 p-5 rounded-2xl flex flex-col gap-4" style={{ background: "linear-gradient(135deg, #0d1f3c, #070d1e)", border: "1px solid rgba(28,50,84,0.8)" }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p style={{ fontSize: "11px", color: "#38bdf8", fontFamily: "JetBrains Mono, monospace" }}>
              Target: {useDemoData ? "192.168.1.0/28 Subnet (Demo)" : targetIp} · {nodes.length} Host Nodes · {edges.length} Pivot Edges
            </p>
          </div>
        </div>
        ...
        <button
          ...
          style={{ background: useDemoData ? "rgba(56,189,248,0.2)" : "rgba(56,189,248,0.08)", fontFamily: "Inter" }}
        >
          <Eye size={13} /> {useDemoData ? "Viewing Demo Subnet (4 Hosts)" : "View Demo Subnet Graph"}
        </button>
      </div>
